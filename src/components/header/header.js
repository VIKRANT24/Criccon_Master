...
//local img
import shapes1 from '../../assets/images/logo/logo.png'
import shapes2 from '../../assets/images/logo/logo.png'
import shapes3 from '../../assets/images/logo/logo.png'
import shapes4 from '../../assets/images/logo/logo.png'
import shapes5 from '../../assets/images/logo/logo.png'
import avatars1 from '../../assets/images/header/profile.png'
...
const Header = (() => {
    ...
    let history = useNavigate(0)
    const user_name = sessionStorage.getItem('full_name')
    ...
    const logout = () => {
        ...
        signOut(auth).then(
          (data) => {
            ...
            Alert('00', 'Logout successful.')
            store.dispatch(loadingToggleAction(false))
            console.log('==================================')
            //window.location.href = 'http://criconn.tenniscricket.in/auth/login';
            history('/auth/login')
            sessionStorage.clear()
          }
        ).catch((err) => {
          console.log('Response: '+JSON.stringify(err))
          Alert('01', 'Error in logout.')
          store.dispatch(loadingToggleAction(false))
          console.log('==================================')
        });
            ...
            <Navbar expand="lg" variant="light" className="nav iq-navbar">
                <Container fluid className="navbar-inner">
                    <Link to="/home/dashboard" className="navbar-brand">
                        <Image src={logo} height="30px" width="30px" alt="images" />
                        <h4 className="logo-title">TennisCricket.in</h4>
                    </Link>
                            ...
                            <Dropdown as="li" className="nav-item">
                                <Dropdown.Toggle as={CustomToggle} variant=" nav-link py-0 d-flex align-items-center" href="#" id="navbarDropdown" role="button" data-bs-toggle="dropdown" aria-expanded="false">
                                    ...
                                    <div className="caption ms-3 d-none d-md-block ">
                                        <h6 className="mb-0 caption-title">{user_name}</h6>
                                        {sessionStorage.getItem('user_role')==='3'?<p className="mb-0 caption-sub-title">Sub-Admin</p>:''}
                                        {sessionStorage.getItem('user_role')==='2'?<p className="mb-0 caption-sub-title">Scorer</p>:''}
                                        {sessionStorage.getItem('user_role')==='1'?<p className="mb-0 caption-sub-title">Admin</p>:''}
                                    </div>
                                </Dropdown.Toggle>
